...
class _RoutineEditorScreenState extends ConsumerState<RoutineEditorScreen> {
                      ...
                      : ReorderableListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return _TemplateExerciseCard(
                              ...
                              exercise: e,
                              onRemove: () =>
                                  _removeExercise(e.templateExercise.id),
                              onUpdateSets: (sets) => _updateTemplateExercise(
                                e.templateExercise.id,
                                targetSets: sets,
                              ),
                              onUpdateReps: (reps) => _updateTemplateExercise(
                                e.templateExercise.id,
                                targetReps: reps,
                              ),
                              onUpdateWeight: (w) => _updateTemplateExercise(
                                e.templateExercise.id,
                                targetWeight: w,
                              ),
                            );
  ...
  Future<void> _editName(BuildContext context, String currentName) async {
    ...
    if (newName != null && newName.isNotEmpty) {
      await ref
          .read(routineDaoProvider)
          .updateRoutine(widget.routineId, name: newName);
  ...
  Future<void> _addExercise(BuildContext context) async {
    ...
    if (result != null && result.$1.isNotEmpty) {
      ...
      final detail = await ref.read(
        routineDetailProvider(widget.routineId).future,
      );
  ...
  Future<void> _removeExercise(int templateExerciseId) async {
    ...
    await ref
        .read(routineDaoProvider)
        .removeExerciseFromRoutine(templateExerciseId);
    ref.invalidate(routineDetailProvider(widget.routineId));
  }

  Future<void> _updateTemplateExercise(
    int id, {
    int? targetSets,
    int? targetReps,
    double? targetWeight,
  ...
  Future<void> _reorderExercises(
    List<RoutineExerciseWithDetails> exercises,
    int oldIndex,
    int newIndex,
  ) async {
    ...
    for (int i = 0; i < reordered.length; i++) {
      await dao.updateTemplateExercise(
        reordered[i].templateExercise.id,
        orderIndex: i,
      );
    }
    ref.invalidate(routineDetailProvider(widget.routineId));
      ...
      child: Row(
        ...
        children: [
          ...
          Padding(
            ...
            child: Text(
              isDouble
                  ? '${(value / 2).toStringAsFixed(1)} $label'
                  : '$value $label',
              style: const TextStyle(
                fontSize: 12,
...
class _ExercisePickerForRoutine extends ConsumerStatefulWidget {
  @override
  ConsumerState<_ExercisePickerForRoutine> createState() =>
      _ExercisePickerForRoutineState();
}

class _ExercisePickerForRoutineState
    extends ConsumerState<_ExercisePickerForRoutine> {
              ...
              data: (exercises) => ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    subtitle: Text(
                      '${_cap(ex.primaryMuscleGroup)} · ${_cap(ex.equipment)}',
                      style: const TextStyle(
                        fontSize: 12,
                        color: AppColors.textTertiary,
                      ),
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                          'Add ${_selectedIds.length} Exercise${_selectedIds.length > 1 ? 's' : ''}${_isSuperset ? ' as Superset' : ''}',
                          style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                          ),
